...
;(function($){
	$.extend($.fn, {
		onePageScroll: function(options){
			...
			percent = Math.ceil(Math.ceil((100*100000)/total)/10)/10000,
			status = "off",
			topPos = 0,
			lastAnimation = 0,
			quietPeriod = 300,
			...
			$.fn.transformPage = function(settings, pos, index, next_el) {
				...
				//var translate = {translate3d: "0, " + pos + "%, 0"};
				var translate = getTransforms("translate3d(0, " + pos + "%, 0)");
				el2.animate(translate
			...
			$.fn.moveUp = function() {
				...
				if(next.length < 1) {
					if (settings.loop == true) {
						pos = ((total - 1) * percent) * -1;
						next = $(settings.sectionContainer + "[data-index='" + total + "']");
					} else {
						return
					}
				}else {
					pos = ((next.data("index") - 1) * percent) * -1;
				}
				var next_index = next.data("index");
				current.removeClass("active")
				next.addClass("active")
			...
			el.css("position","relative").css('height', 100*total+'%');
			$.each(sections, function(i, item){
				//.css('height', percent+'%');
				$(this).addClass("ops-section").data("index", i + 1).css('height', percent+'%');
				topPos = topPos + 100;
			...
			if(window.location.hash != "" && window.location.hash != "#1" && window.location.hash != "#rd") {
				...
				var pos = ((init_index - 1) * percent) * -1;
